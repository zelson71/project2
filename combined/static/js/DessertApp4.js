d3.csv("Resources/Desserts.csv").then(function (tableData) {
    ...
    function buildScatterPlot(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            // row = RedsData[i];
            var hover = `${Title[i]}<br><b>Vintage:</b> ${Vintage[i]}<br><b>Rating:</b> ${Rating[i]}<br><b>Price:</b> $${Price[i]}`
    ...
    button.on("click", () => {
        ...
        // }
        console.log(d3.select("#selPrice").property("value"));
        ...
        //   };
        // console.log(bouncer(tableData.rating));
        ...
        if (!Array.isArray(filteredArray) || !filteredArray.length)
            ...
            buildTable(filteredArray3),
                buildScatterPlot(filteredArray3)
    });
})
